...
if (document.activeElement !== inputs.alpha) {
inputs.alpha.value = camera.alpha.toFixed(4);
}

if (document.activeElement !== inputs.beta) {
inputs.beta.value = camera.beta.toFixed(4);
}

if (document.activeElement !== inputs.radius) {
inputs.radius.value = camera.radius.toFixed(4);
}

if (document.activeElement !== inputs.fov) {
inputs.fov.value = (camera.fov * 180 / Math.PI).toFixed(2);
}

if (document.activeElement !== inputs.x) {
inputs.x.value = camera.target.x.toFixed(4);
}

if (document.activeElement !== inputs.y) {
inputs.y.value = camera.target.y.toFixed(4);
}

if (document.activeElement !== inputs.z) {
inputs.z.value = camera.target.z.toFixed(4);
}
}
...
const end = {
alpha: randomRange(0, Math.PI * 2),
beta: randomRange(70 * Math.PI / 180, 110 * Math.PI / 180),
radius: randomRange(5, 30)
};
...
1,
(now - startTime) / RANDOM_CAMERA_DURATION
);
...
camera.alpha =
  start.alpha +
  (end.alpha - start.alpha) * eased;

camera.beta =
  start.beta +
  (end.beta - start.beta) * eased;

camera.radius =
  start.radius +
  (end.radius - start.radius) * eased;

if (progress < 1) {
  randomCameraAnimation =
...
z * 8
  ...
  const bulbMaterial = new StandardMaterial(
    `lightBulbMaterial_${x}_${z}`,
    scene
  );

  bulbMaterial.disableLighting = true;
  bulbMaterial.emissiveColor =
    lightColor.scale(LIGHT_BULB_EMISSIVE);
...
// Dedicated rotation root.
// Keep the GLB hierarchy intact underneath it.
modelRoot = new TransformNode(
"modelRotationRoot",
scene
);

const roots = result.meshes.filter(
...
rotationSpeed +=
(rotationTarget - rotationSpeed) *
Math.min(1, ROTATION_ACCELERATION * delta);
